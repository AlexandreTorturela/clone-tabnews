  ...
  describe('Anonymous user', () => {
    test('With nonexistent token', async () => {
      const response = await fetch(
        'http://localhost:3000/api/v1/activations/a5453213-d2f3-4090-940d-b3983bb877f6',
        { method: 'PATCH' }
      );
    ...
    test('With valid token', async () => {
      ...
      expect(expiresAt - createdAt).toBe(activation.EXPIRATION_IN_MILLISECONDS); //pode quebrar em operação
    ...
    test('With valid token but already activated user', async () => {
      ...
      expect(responseBody).toEqual({
        ...
        message: 'Você não pode utilizar mais tokens de ativação.',
